...
import "./storage/PlennyOceanStorage.sol";
...
contract PlennyOcean is PlennyBasePausableV2, PlennyOceanStorage {
    ...
    function initialize(address _registry) external initializer {
        // in block numbers
        cancelingRequestPeriod = 6500;
        // 50 PL2
        makerCapacityOneTimeReward = uint256(50).mul(10 ** 18);
        // 2.5%
        takerFee = 250;
        // 2.5%
        makerRewardFee = 250;
    ...
    function requestLightningCapacity(string calldata nodeUrl, uint256 capacity, address payable makerAddress,
        ...
        require(staking.plennyBalance(owner) >= openingCapacity + fee, "ERR_NO_FUNDS");
        ...
        emit CapacityRequestPending(owner, capacity, makerAddress, capacityRequestsCount, openingCapacity + fee);
    }
    ...
    function cancelRequestLightningCapacity(uint256 capacityRequestIndex) external whenNotPaused nonReentrant {
        ...
        uint256 sec = _blockNumber() - capacityRequest.addedDate;
    ...
    function processCapacityRequest(uint256 capacityRequestIndex) external override whenNotPaused nonReentrant onlyCoordinator {
        ...
        capacityRequest.plennyReward -= makerCapacityOneTimeReward;
        ...
        _approve(address(this), makerCapacityOneTimeReward - makerOneTimeRewardFee);
        ...
        token.safeTransferFrom(address(this), capacityRequest.makerAddress,
            makerCapacityOneTimeReward - makerOneTimeRewardFee);
    }
    ...
    function updateMakerProvidingAmount(uint256 makerIndex, uint256 amount, bool incrementDecrement) internal {
        ...
        if (incrementDecrement) {
            maker.makerProvidingAmount += amount;
        } else {
            maker.makerProvidingAmount -= amount;
        }
    }
    ...
    function _collectCapacityRequestReward(uint256 capacityRequestIndex, uint256 channelId, uint256 confirmedDate)
        ...
        uint256 _reward = _blockNumber() - confirmedDate > factory.userChannelRewardPeriod() ?
        capacityRequest.plennyReward.mul(factory.userChannelReward())
        .mul(_blockNumber() - contractRegistry.coordinatorContract().channelRewardStart(channelId))
        ...
        capacityRequest.plennyReward -= _reward;
        uint256 rewardFee = _reward.mul(makerRewardFee).div(100).div(100);

        IPlennyERC20 token = contractRegistry.plennyTokenContract();
        _approve(address(this), _reward - rewardFee);
        token.safeTransfer(contractRegistry.requireAndGetAddress("PlennyRePLENishment"), rewardFee);
        token.safeTransferFrom(address(this), capacityRequest.makerAddress, _reward - rewardFee);
