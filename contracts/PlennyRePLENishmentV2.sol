...
/// @title  Plenny rePLENishment version 2
...
contract PlennyRePLENishmentV2 is PlennyBasePausableV2, PlennyFeeStorage {
	...
	function sellEthForPlenny(uint256 jobId, uint256 ethAmount) private returns (uint256 ethSold, uint256 plennyReceived) {
		...
		if (amounts[amounts.length - 1] < plennyQuoteOut) {
			plennyQuoteOut = amounts[amounts.length - 1];
		}
