...
contract PlennyValidatorElectionV2 is PlennyBasePausableV2, PlennyValidatorElectionStorage {
    ...
    function newElection() external whenNotPaused nonReentrant {
        ...
        (uint256[] memory immutableScores, ) = factory.getValidatorsScore();
        ...
        for (uint i = 0; i < newValidators.length; i++) {
            ...
            electionsArr[latestElectionBlock][newValidators[i]] = ElectionInfo(latestElectionBlock, stakedBalance, immutableScores[validatorIndex]);
            currentElection[latestElectionBlock].push(ElectionInfo(latestElectionBlock, stakedBalance, immutableScores[validatorIndex]));
        }
